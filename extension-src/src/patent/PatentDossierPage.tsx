...
const AiSummarySection: React.FC<AiSummarySectionProps> = ({ summary, loading, error, onGenerate }) => (
  <Section
    num={8}
    title="AI Summary"
    intro="Plain-English executive overview and claim-scope characterization generated by Gemini. Generated summaries are cached for 30 days; regenerate to refresh."
  >
    <div className="border border-slate-200 rounded-md p-4 bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="flex items-center justify-between mb-3">
        ...
        {summary ? (
          ...
          <button
            onClick={onGenerate}
            disabled={loading}
            className="text-[11px] font-semibold px-3 py-1.5 rounded bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:opacity-90 disabled:opacity-60 flex items-center gap-1.5"
          >
      ...
      {!summary && !loading && !error && (
        <p className="text-xs text-slate-500 italic">
          Click <strong>Generate</strong> to produce a 2–3 paragraph plain-English overview plus a claim-scope
          characterization (narrow / moderate / broad). Costs 1 credit; cached for 30 days.
        </p>
      )}
    </div>
  </Section>
