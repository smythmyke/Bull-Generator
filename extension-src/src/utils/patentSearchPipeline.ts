import { optimizeQuery, enrichNPL, EnrichedNPLItem, analyzeRound, AnalyzeRoundResponse, CPCSuggestion, TerminologySwap, ConceptHealth } from "../services/apiService";
 ...
 * truncation wildcards (word* -> word), proximity operators (NEAR/3, 2D, 3D).
 */
export function sanitizeForGooglePatents(query: string): string {
  let q = query;

  // Remove field prefixes: FT=, TAC=, AB=, TI=, CL=, CA=
  q = q.replace(/\b(?:FT|TAC|AB|TI|CL|CA|CPC)\s*=/gi, "");

  // Remove country code clause: AND CC=XX (with optional parens/spaces)
  q = q.replace(/\s+AND\s+CC\s*=\s*[A-Z]{2}/gi, "");

  // Remove truncation wildcards: word* -> word
  q = q.replace(/(\w)\*/g, "$1");

  // Remove proximity operators: NEAR/N, nD (2D, 3D)
  q = q.replace(/\bNEAR\/\d+\b/gi, "AND");
  q = q.replace(/\b\d+D\b/gi, "AND");
...
export async function runTripleSearch(params: TripleSearchParams): Promise<void> {
  ...
  const [optimizedResult, search1Results] = await Promise.all([
    ...
    runSingleSearch(tabId, rawText, 15),
  ]);
  ...
  const search2Results = await runSingleSearch(tabId, gpBooleanQuery, 15);
  ...
  if (optimizedResult.optimizedQuery) {
    ...
    search3Results = await runSingleSearch(tabId, optimizedResult.optimizedQuery, 15);
...
export async function runProAutoSearch(params: ProAutoSearchParams): Promise<void> {
  ...
  const [optimizedResult, rawTextResults] = await Promise.all([
    ...
    runSingleSearch(tabId, smartRawText, 15),
  ]);
  ...
  if (optimizedResult.optimizedQuery) {
    ...
    if (tabOkAI) {
      ...
      aiResults = await runSingleSearch(tabId, optimizedResult.optimizedQuery, 15);
  ...
  for (let i = 0; i < topPatentIds.length; i++) {
    ...
    const simResults = await runSingleSearch(tabId, `~patent/${patentId}`, 15);
...
export async function runProInteractiveSearch(params: ProInteractiveSearchParams): Promise<void> {
  ...
  if (optimizedResultI.optimizedQuery) {
    ...
    if (tabOkAII) {
      ...
      aiResultsI = await runSingleSearch(tabId, optimizedResultI.optimizedQuery, 15);
