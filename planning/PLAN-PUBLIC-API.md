...
## Future distribution surfaces

- **RapidAPI (Rapid Hub) — PLANNED, good niche fit (added 2026-05-29).** Patent/legal-research APIs have a real, if niche, developer market on RapidAPI (legaltech, research tooling). Once the v1 REST surface above is stable, `/v1/dossier` (+ the free bundled endpoints) is a clean stateless contract to list. **RapidAPI is a *REST* marketplace, not an MCP one** — it's a parallel surface to the MCP registries (Smithery/Glama/Official Registry where patent-search already lives), not a replacement; the MCP server stays the primary v1 consumer that drives the contract. Reuses the existing `X-API-Key: psg_live_` auth — map the RapidAPI gateway key → owning `uid`/credit pool at the boundary, distinct User-Agent for attribution, no parallel billing system. **Build trigger:** after the v1 public API + MCP server ship and prove reliable. **Caveats:** **flat 25% RapidAPI marketplace fee** (keep 75%) on top of processor fees, **PayPal-only payout** consolidated monthly ~5–6 weeks after charge (additive-discovery channel, not primary billing); another versioned REST spec + docs to maintain; RapidAPI's softening listing-quality reputation. Lower volume than a horizontal SEO API (cf. JackpotKeywords), but low effort to add on top of the v1 work. **Billing mechanism (from RapidAPI docs):** define a `Credits` custom quota object and return `X-RapidAPI-Billing: Credits=<n>` per response (e.g. `/v1/dossier` → `Credits=3`) so RapidAPI meters against JK-style per-call credit costs; validate the `X-RapidAPI-Proxy-Secret` header server-side so the direct CF URL can't bypass the gateway. See JackpotKeywords' plan for the fully-worked pilot — patent-search reuses that playbook.
