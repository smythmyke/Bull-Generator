...
4. ◐ Phase 4 — summary + claim-chart un-gated onto ODP ✅ 2026-05-31 (additive `fetchDossier` injection; API/MCP pass `handleOdpDossierRequest`, extension omits it → GP read-through unchanged; both verified green). **`/v1/search` ✅ DONE 2026-05-31** — marketplace path returns AI-generated Boolean queries (`executionMode=client_side`), no Google execution; extension's Firebase path unchanged. **Public surface now 100% scraping-free.** ODP has no full-text search (verified) so server-side full-text execution is deferred to API-FT-1 (PatentsView, transition risk) — see `memory/research_v1_search_fulltext_source.md`.
...
7. ◐ Phase 6 — RapidAPI. **6a backend shim BUILT 2026-05-31** (JK-consistent): `functions/src/rapidapi.ts` (proxy-secret validate + Credits cost map [1 Credit=1¢] + billing-header patch); `credits.ts` += `billingExempt` skip + `rapidapi` source; `auth.ts` += `rapidapi` AuthSource; `index.ts` RapidAPI branch (house account, ODP path, rate-limit bypass). House account provisioned (`scripts/provision-rapidapi-house.js`, uid `rapidapi-house`). OpenAPI spec `docs/api-deployment/openapi-rapidapi.yaml`. Keeps Bull-Generator's own credit system intact (NOT JK's separate cents accounts — see decision below). **Remaining (user): create RapidAPI listing → copy proxy secret → set `PSG_RAPIDAPI_PROXY_SECRET`+`PSG_RAPIDAPI_HOUSE_UID` env → redeploy → §5b gateway test. Pricing/plans set in RapidAPI console.**
